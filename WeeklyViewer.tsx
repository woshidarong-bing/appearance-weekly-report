...
export default function WeeklyViewer({ weekId }: { weekId?: string }) {
  ...
  useEffect(() => {
    ...
    if (!supabase) {
      setReport(weekId ? getLocalReport(weekId) : getLatestLocalReport());
      setLoading(false);
      return;
    }
    let query = supabase.from("weekly_reports").select("*").eq("status", "published");
    query = weekId ? query.eq("week_id", weekId).limit(1) : query.order("period_end", { ascending: false }).limit(1);
  ...
  if (!report) return <><AppHeader compact/><main className="public-empty"><div className="empty-illustration">周</div><h1>暂无已发布周报</h1><p>{weekId ? "该期周报不存在或尚未发布。" : "最新周报发布后会显示在这里。"}</p></main></>;
